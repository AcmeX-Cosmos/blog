...
**实际行为**：速度估计偏大，而且对观测噪声的响应比预期快得多——单帧观测跳一下，速度立刻跟着跳。调小 `sigma2_q` 想让它更信模型，[结果发现那组参数压根没接上](/blog/ekf-qr-adaptive-tuning)；名字改对之后症状缓解了，但仍然比理论值\"活泼\"。
